...
func TestCompilerScopes(t *testing.T) {
	...
	if last.Opcode != code.OpSub {
		t.Errorf("last instruction opcode wrong. got=%d, want=%d", last.Opcode, code.OpSub)
	}
	...
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 2 {
		t.Errorf("instructions length wrong. got=%d, want=2", len(compiler.scopes[compiler.scopeIndex].instructions))
	}

	last = compiler.scopes[compiler.scopeIndex].lastInstruction
	if last.Opcode != code.OpAdd {
		t.Errorf("last instruction opcode wrong. got=%d, want=%d", last.Opcode, code.OpAdd)
	}
